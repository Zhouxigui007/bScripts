...
function scriptContainer(){
	...
	function findSmallestWidth(){
		if(smallestSize == "XXS" || smallestSize == "S" || smallestSize == "YS"){
			smallestWidth = 10.8;
			smallestScale = 3;
			smallestLogo = artLayers.layers.getByName("Front Logo").pageItems[0].height;
			secondSmallestLogo = smallestLogo + 3.6;
		}
	...
	function placeFrontLogo(){
		...
		for(var a=0;a<prepressLayer.layers.length;a++){
			...
			if(wearerLayer.name.substring(0,13) != "FD_BASE_FB_SS" && wearerLayer.name.substring(0,15) != "FD_BASE_FB_Y_SS" && wearerLayer.name.substring(0,15) != "FD_FAST_FB_W_SS" && //
				wearerLayer.name.substring(0,15) != "FD_BASE_FB_Y_SL" && wearerLayer.name.substring(0,13) != "FD_BASE_FB_SL"){
...
// 				logoResize.top = logoTop;
	...
	function placeFrontNumber(){
		...
		var vertPos = secondSmallestLogo - smallestLogo;
		var topStart = artLayers.layers.getByName("Front Logo").pageItems[0].height-smallestLogo;
		var top = fNumber.top + vertPos;
		
		
		if(fNumberLoc == '1'){
			left = fNumber.left + (smallestWidth/2);
			top = fNumber.top + topStart;
			
			for(var a=0;a<prepressLayer.layers.length;a++){
				var curSize = prepressLayer.layers[a].name;
				if(wearerLayer.name.indexOf("FB")==-1){
