...
Progress:
	...
	Version: 1.009
		01 June, 2016
		Updated scriptUI dialog to include a cancel button that halts execution completely.
		Added the "currently selected" garments to the scriptUI window so the user knows what they've chosen.
		Included a check for selected collars before script UI runs. If no selection. alert and halt execution.
		Make "DR Collars" label selected upon completion of script along with the newly added collars.

	Version: TESTING
		01 June, 2016
		This version has been distributed to artists for beta testing.
		The name on the network is "Get_Collars.jsx".


	Version 1.010
		31 October, 2016
		Updated paths with new network file system locations.
...
function container()
{
		...
		try
		{
			...
			for(var a=0;a<layers.length;a++)
			{
				if(layers[a].name.indexOf("FD")>-1)
					...
					//check for existence of prepress layer
					//this is to fix the issue of a non-converted-template that has "FD" in the name throwing an error.
					try
					{
						if(layers[a].layers["Prepress"])
						{
							$.writeln("no errors finding the prepress layer. sending this layer to list of template layers.")
						}
					}
					catch(e)
					{
						$.writeln("reached catch statement. returning false");
						return false;
					}
	...
	function getObjectData(thisTemplate, library)
	{
		...
		try{
			...
			if(theCode == "FD_FAST_2B_W")
			{
				thisTemplate.name = "FD_FAST_2B_SS_" + result.styleNum;
				result.code = "FD_FAST_2B_SS";
			}
			else
			{
			...
			if(thisTemplate.name.indexOf("SLOW")>-1)
			{
				var regRag;

				for(var rr=0;rr<thisTemplate.layers["Information"].layers.length;rr++)
				{
					var thisLay = thisTemplate.layers["Information"].layers[rr];
					if(thisLay.name.indexOf("Raglan")>-1)
					{
						regRag = "Raglan";
					}
					else if(thisLay.name.indexOf("Regular")>-1)
					{
						regRag = "Regular";
					}
					else
					{
						//couldn't automatically determine regular or raglan. prompting user with script ui
						...
						function regRagUi()
						{
							...
							var rrw = new Window("dialog", "Regular or Raglan?");
								var msgGroup = rrw.add("group");
									var msg = msgGroup.add("statictext", undefined, "For the garment: " + thisTemplate.name + "\nIs it Regular or Raglan?");

								var btnGroup = rr.add("group");
									var regButton = btnGroup.add("button", undefined, "Regular");
									...
									var ragButton = btnGroup.add("button", undefined, "Raglan");
									ragButton.onClick = function()
									{
										uiResult = "Raglan";
										rrw.close();
										return uiResult;
				...
				result.path = library["network path"] + library[theCode][regRag];
			}

			//garment is not slowpitch. no need to check for regular or raglan
			else
			{
				result.path = library["network path"] + library[theCode]["path"];
				$.writeln("result.path = " + result.path);
			}
	...
	function getNewCollars(code,path,coords,style)
	{
		...
		drLabel.selected = true;
	}

	function whichGarments(currentGarments)
	{
		...
		if(currentGarments.length > 0)
		{
			var templateListDisplay = wS.add("group");
				...
				var TLDLabel = templateListDisplay.add("statictext", undefined, "Grabbing collars for:");
				for each(shirt in currentGarments)
				{
	...
	var library =
			{
				"network path" : "/Volumes/Customization/Library/cads/prepress/",
				//this object stores all relevant data for the network
				//location and placement coords of DR collars
				"FD_SLOW_SS" : 
				{
					"path": "DIAMOND SPORTS/FD-161_Men's_Short Sleeve Tee-Raglan Sleeve/_Converted Templates",
					"coords" : 
					{
						"XS" : [-1133.12,625.48],
						"S" : [-1135.37,366.12],
				...
				"FD_SLOWW_SS" : 
				{
					"path" : "DIAMOND SPORTS/FD_SLOW_SS/ConvertedTemplates/FD_SLOWW_SS/",
					"coords" : 
					{
						"XXS" : [-904.86,482],
						"XS" : [-906.86,242],
				...
				"FD_SLOWY_SS" :
				{
					"path" : "DIAMOND SPORTS/FD_SLOW_SS/ConvertedTemplates/FD_SLOWY_SS/",
					"coords" : 
					{
						"YXS" : [-803.12,-39.21],
						"YS" : [-807.12,-238.73],
				...
				"FD_210_211" :
				{
					"path" : "BASKETBALL/FD-210-FD-211_Mens_Basketball_Uniform/_Converted_Templates/",
					"coords" :
					{
						"XS" : [-1133.122,625.479],
						"S" : [-1135.372,366.119],
						...
						"L" : [1139.872,-172.141],
						"XL" : [-1141.672,-453.052],
						"2XL" : [-1144.372,-740.871],
						"3XL" : [-1146.622,-1036.672],
						"4XL" : [-1148.872,-1339.05],
						"5XL" : [-1151.122,-1648.31],
						"Label" : [-1116.6,12.33]
					}
				}
			}
	...
	else if(valid && !existTemplate)
	{
		...
		//Collars must be selected at this point..
		//Check that selection is > 0.

		if(docRef.selection.length<1)
		{
			errorList.push("You must first select the USA Collars.\nPlease try again");
			sendErrors(errorList);
			return;
		}

		//At least SOME collars are correctly selected
		//Proceed.
		var usaCollars = findUSACollars("sel");
		theDocument.usaCollars = usaCollars;
		
		//Begin scriptUI function loop to prompt user for all needed collars.
		...
		while(cont)
		{
			var theResult = whichGarments(templateList);
			if(theResult == "cancelled")
			{
				templateList = null;
				valid = false;
				return;
