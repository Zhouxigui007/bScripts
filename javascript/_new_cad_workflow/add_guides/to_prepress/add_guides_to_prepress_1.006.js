...
Progress:
	...
	Version 1.003
		28 February, 2017
		Fixed a bug that was causing only one document to process.
		Batch is now working properly.

	Version 1.004
		02 March, 2017
		Fixing an issue regarding guides that are group items.
			Need to dig into groupItems or compoundPath items to determine whether item is a guide
			guides is not a property of a groupItem or compoundPathItem.
		This will only work if the group is composed of paths and not composed of nested groups.
		Tested and working. Distributing to artists.

	Version 1.005
		02 March, 2017
		Adding specific error message for the case when a guide group is comprised entirely of groups.

	Version 1.006
		06 March, 2017
		Fixed a bug that caused an error due to disorganized grouping structure. Instead of throwing an error message when
			a group is comprised of groups, simply skip it quietly. It'll be up to the user to check that everything worked properly.
*/
		...
		for(var pp=0;pp<ppLay.layers.length;pp++)
		{
			...
			for(var cs=0;cs<thisLay.groupItems.length;cs++)
			{
				...
				for(var pi=0;pi<thisShirtPiece.pageItems.length;pi++)
				{
					...
					var isGuide;
					if(thisItem.typename == "GroupItem" && thisItem.pathItems.length == 0)
					{
						// alert("You cannot use a guide that is made up entirely of groups. Please ungroup all the way, then regroup the individual path items.\nSee William with questions.");
						// return false;
						continue;
					}
					if((thisItem.name != "Prod Info" && thisItem.name != "Artwork") && thisItem.typename == "GroupItem" || thisItem.typename == "CompoundPathItem")
					{
						isGuide = thisItem.pathItems[0].guides;
					}
					else
					{
						isGuide = thisItem.guides;
					}
					if(isGuide)
					{
			...
			var btnGroup = w.add("group");
				...
				allButton.onClick = function()
				{
					...
					while(app.documents.length>1)
					{
						...
						$.writeln("activeDocument = " + app.activeDocument.name);
